...
def is_writable(schema):
    """Determine if a field can have a value written by user"""
    if "$ref" in schema:
        return True  # user can fill value for ref
    schema_type = schema.get("type")
    return schema_type in ["string", "number", "boolean"]
...
def schema_to_excel(schema_file, excel_file):
    ...
    headers = ["Path", "Type", "Required", "Description", "Ref", "Writable", "Value"]
    ws.append(headers)

    for field in fields:
        ws.append([field.get(h, "") for h in headers[:-1]] + [""])  # empty Value col
...
# Example usage
schema_to_excel("../files/input/FacilityRecord.json", "../files/output/dfr_schema_mapping_v4.xlsx")
